...
fn generate_and_print_forms(
    ...
    let cases = vec![
        (
            "First Case",
            match ending {
                'i' | 'u' => vec![("", i_to_I_u_to_U)],  // Add parentheses to make it a proper tuple
                'a' => vec![("o", no_vowel)],
                _ => vec![],
            },            
            match ending {
                'a' => vec![("", a_to_A)],
                'i' => vec![("au", no_vowel), ("ao", no_vowel), ("", i_to_I_u_to_U), ("No", original)],
                'u' => vec![("au", no_vowel), ("ao", no_vowel), ("", i_to_I_u_to_U), ("No", original), ("avo", no_vowel)],
                _ => vec![],
            },
        ),
        (
            "Second Case",
            vec![("M", original)], // Singular forms
            match ending {
                'a' => vec![("", a_to_A), ("", a_to_e)], // For a-ending words
                'i' | 'u' => vec![(i_to_I_u_to_U, ""), ("No", original)], // Corrected for i/u-ending words
                _ => vec![],
            },
        ),
